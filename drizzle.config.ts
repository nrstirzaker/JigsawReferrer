...
function getURL(){
    ...
    const password : string = process.env.DATABASE_PASSWORD ? process.env.DATABASE_PASSWORD : "";
    const databaseName : string = process.env.DATABASE_NAME ? process.env.DATABASE_NAME : "";
    const awsSupabase : string = process.env.DATABASE_AWS_SUPABASE ? process.env.DATABASE_AWS_SUPABASE : "";

    const url : string = 'postgresql://'+username+'.'+host +':'+password +'@'+ awsSupabase +':'+ port +'/'+databaseName
